...
namespace g_game
{
	...
	namespace g_draw
	{
		...
		void draw_bones(Vector2* bones_screenPos, long count, DWORD color, float tickness)
		{
			long last_count = count - 1;
			for (long i = 0; i < last_count; ++i)
				g_draw::draw_line(ImVec2(bones_screenPos[i].x, bones_screenPos[i].y), ImVec2(bones_screenPos[i + 1].x, bones_screenPos[i + 1].y), color, tickness);
		}
		...
		void draw_bones(unsigned long i, Vector3 origin, DWORD color)
		{
				...
				sdk::get_bone_by_player_index(i, sdk::BONE_POS_RIGHT_HAND_4, &left_hand[4]))
			{
				...
				draw_bones(screen_header_to_bladder, 6, color, globals::bonethick);
				draw_bones(screen_right_foot_to_bladder, 5, color, globals::bonethick);
				draw_bones(screen_left_foot_to_bladder, 5, color, globals::bonethick);
				draw_bones(screen_right_hand, 5, color, globals::bonethick);
				draw_bones(screen_left_hand, 5, color, globals::bonethick);
				if (globals::Outline)
				{
					draw_bones(screen_header_to_bladder, 6, 0xFFFFFF00, globals::outlinethick);
					draw_bones(screen_right_foot_to_bladder, 5, 0xFFFFFF00, globals::outlinethick);
					draw_bones(screen_left_foot_to_bladder, 5, 0xFFFFFF00, globals::outlinethick);
					draw_bones(screen_right_hand, 5, 0xFFFFFF00, globals::outlinethick);
					draw_bones(screen_left_hand, 5, 0xFFFFFF00, globals::outlinethick);
				}
			}
		}
