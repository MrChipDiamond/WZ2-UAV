...
namespace globals
{
	extern float outlinethick;
	extern float bonethick;
	extern bool b_radar;
	extern  const char* R_MOdes[];
	extern  const char* R_MOdes_2[];
	extern int Selected_R_mod3;
	extern int Selected_R_mod3_2;
	extern bool b_rainbow;
	extern bool b_itemglow;
	extern bool radar;
	...
	extern float f_fov;
	extern bool b_third;
	extern bool b_heartbeat;
	extern bool b_bright;
	extern bool b_noflashbang;
	extern bool b_sprint;
	extern bool Outline;
	extern bool is_Unlock_Written_To;
	extern bool b_unlock;
	extern bool b_pinguav;
	extern float f_minimap;
	extern uintptr_t Dvar_FindVarByName;
	extern uintptr_t Dvar_SetBoolInternal;
	extern uintptr_t Dvar_SetBoolByName;
	extern uintptr_t Dvar_SetFloat_Internal; //48 8B 0D ? ? ? ? E8 ? ? ? ? 4C 8B 15 ? ? ? ? 49 8B 4A 28 C6 84 24 ? ? ? ? ? 
	extern uintptr_t Dvar_RegisterFloat;
	extern uintptr_t Dvar_SetInt_Internal;
	extern uintptr_t Dvar_SetIntByName;
	extern uintptr_t unlockall;
	extern uintptr_t clantagbase;
	extern uintptr_t camobase;
	extern uintptr_t unlockallnew;
	...
	extern uintptr_t uavbase;
	extern uintptr_t ShowToastNotificationAfterUserJoinedParty;

	extern int i_score; //1 ~ 5555555555
	extern int i_timeplayed;
	extern int i_games; //1 ~ 5555555555
	extern int i_wins; //1 ~ 5555555555
	extern int i_kills; //1 ~ 5555555555
	extern int i_deaths; //1 ~ 5555555555
	extern int i_downs; //1 ~ 5555555555;
	extern int i_top5; //1 ~ 5555555555
	extern int i_top10; //1 ~ 5555555555
	extern int i_top25; //1 ~ 5555555555
